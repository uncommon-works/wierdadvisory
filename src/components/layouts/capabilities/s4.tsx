    ...
    <section ref={sectionRef} className="relative h-auto md:h-[100vh] bg-blue-50">
      <div className="sticky top-0 flex flex-col items-center justify-center min-h-screen w-full overflow-hidden px-4">
        ...
        <div className="max-w-5xl mx-auto flex flex-col items-start text-left">
          <h2 ref={headingRef} className="text-4xl md:text-6xl text-priamry leading-[1.15] max-w-3xl select-none mb-8">
            {words.map((word, i) => {
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {features.map((feature, index) => (
              <div key={index} className={`${feature.bgColor} rounded-3xl p-8 transition-transform hover:scale-[1.02]`}>
                <div className="space-y-4">
                  <div className="flex items-start gap-3">
                    <feature.icon className={`h-6 w-6 ${feature.textColor}`} />
                    <h2 className={`text-2xl font-semibold ${feature.textColor}`}>{feature.title}</h2>
                  </div>
                  <p className={`text-lg leading-relaxed ${feature.textColor}`}>{feature.description}</p>
                </div>
              </div>
            ))}
          </div>
